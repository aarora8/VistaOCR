...
def get_args():
    ...
    parser.add_argument("--datadir_type", type=str, required=False, help="Path to trained language model for LM decoding", default="test")
    ...
    parser.add_argument("--model-path", type=str, required=True, help="Path to trained OCR model used in decoding")
    parser.add_argument("--cvtGray", default=False, action='store_true', help="Set if need to convert color images to grayscale") 
    parser.add_argument("--lm-path", type=str, required=False, help="Path to trained language model for LM decoding")
...
#        line_img_transforms.append(imagetransforms.HorizontalFlip())
    ...
    test_dataset = OcrDataset(args.datadir, args.datadir_type, line_img_transforms, max_allowed_width=1e5)
    ...
    print("Decoding took %.2fseconds" % (duration))
...
def decode_thread(input_queue, outdir, alphabet, lm_path):
    ...
    with ExitStack() as exit_stack:
        ...
        while True:
            ...
            #print(batch_cnt, hyp_transcriptions)
            ...
            for i in range(len(hyp_transcriptions)):
                ...
                if 'u000a' in hyp_utf8:
                    print('...FOUND bad char u000a in %s, %s ...replacing' % (metadata['utt-ids'][i], hyp_utf8))
                    hyp_utf8 = hyp_utf8.replace('u000a', '')

                hyp_uxxxx = utf8_to_uxxxx(hyp_utf8)
                
                if '\n' in hyp_uxxxx or '\r' in hyp_uxxxx:
                    print('...FOUND bad chars in %s, %s ...replacing' % (metadata['utt-ids'][i], hyp_uxxxx))
                    hyp_uxxxx = hyp_uxxxx.replace('\n', '').replace('\r', '')
